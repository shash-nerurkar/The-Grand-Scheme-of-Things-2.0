...
# warning-ignore:return_value_discarded
		...
		"PATROL":
			...
			timer.paused = true;
			sprite.play("run");
			if global_position.distance_to(patrol_dest) <= 5:
				patrol_dest = Vector2(randf(), randf());
			var coll = move_and_collide(patrol_dest*speed*delta);
			if coll != null:
				patrol_dest = Vector2(randf(), randf());
...
func _on_mughal_mongol_animation_finished():
	if sprite.get_animation() == ("death"):
		...
		single_shoot(enemy);
		timer.start();
	elif sprite.get_animation() == ("reload"):
		STATE_LOCK = true;
		timer.start();
...
func _on_cooldown_timer_timeout():
	STATE = "SHOOT_2";
	STATE_LOCK = false;
	if sprite.get_sprite_frames().has_animation("reload"):
		sprite.play("reload");
		match enemy:
			"vodok":
				vodok_shoot();
...
func vodok_shoot():
	...
	b.scale = scale;
	bullet_container.add_child(b);
	b.start_pos(muzzle.get_angle_to(player.position), muzzle.global_position+Vector2(0,15), 'vodok_small');
	var a = bullet.instance();
	a.scale = scale;
	bullet_container.add_child(a);
	a.start_pos(muzzle.get_angle_to(player.position), muzzle.global_position, 'vodok_big');
